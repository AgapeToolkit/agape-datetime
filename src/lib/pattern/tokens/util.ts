...
export function resolvedDatePartsDateTimeIsoString(parts: ResolvedDateTimeParts, output: 'date' | 'datetime' = 'datetime') {
  const { month=1, day=1, hour = 0, minute = 0, second = 0, nanosecond = 0 } = parts;
  ...
  const time = `${String(hour) === "24"?"00":String(hour).padStart(2,"0")}:${String(minute).padStart(2,"0")}:${String(second).padStart(2, "0")}.${String(Math.floor(nanosecond / 1_000_000)).padStart(3,'0')}`;
  return `${date}T${time}`;
}
