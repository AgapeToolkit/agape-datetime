...
describe('TimeValue', () => {
  describe('Constructor', () => {
    it('should create empty instance when no parts provided', () => {
      ...
      expect(tv.fractionalSecond).toBeUndefined();
    });

    it('should create instance with TimeParts', () => {
      const tv = new TimeValue({ hour: 14, minute: 30, second: 45, fractionalSecond: 0.123 });
      expect(tv.hour).toBe(14);
      expect(tv.minute).toBe(30);
      expect(tv.second).toBe(45);
      expect(tv.fractionalSecond).toBe(0.123);
    });
  ...
  describe('Getters', () => {
    ...
    it('should return set values', () => {
      const tv = new TimeValue({ hour: 14, minute: 30, second: 45, fractionalSecond: 0.123456789 });
      expect(tv.hour).toBe(14);
      expect(tv.minute).toBe(30);
      expect(tv.second).toBe(45);
      expect(tv.fractionalSecond).toBe(0.123456789);
    });
  });
  ...
  describe('from() static method', () => {
    ...
    it('should create from Date object', () => {
      ...
      expect(tv.minute).toBe(30);
      expect(tv.second).toBe(45);
      expect(tv.fractionalSecond).toBeCloseTo(0.123, 3);
    });
    ...
    it('should create from Temporal.PlainTime', () => {
      ...
      const expectedFractional = plainTime.nanosecond / 1_000_000_000;
      expect(tv.fractionalSecond).toBeCloseTo(expectedFractional, 9);
    });
  ...
  describe('from() string parsing', () => {
    ...
    it('should handle fractional seconds with different precision', () => {
      ...
      expect(tv.minute).toBe(30);
      expect(tv.second).toBe(45);
      expect(tv.fractionalSecond).toBeCloseTo(0.7, 1);
    });
  ...
  describe('Temporal conversion methods', () => {
    ...
    it('should convert fractional seconds to nanoseconds', () => {
      const tv = new TimeValue({ hour: 14, minute: 30, second: 45, fractionalSecond: 0.123456789 });
      const plainTime = tv.toPlainTime();
      expect(plainTime.hour).toBe(14);
      expect(plainTime.minute).toBe(30);
